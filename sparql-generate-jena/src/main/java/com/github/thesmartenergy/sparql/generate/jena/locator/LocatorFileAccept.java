 ...
 */
public class LocatorFileAccept implements Locator
{
    // Implementation note:
    // Java7: Path.resolve may provide an answer from the intricies of MS Windows
    
    static Logger log = LogManager.getLogger(LocatorFileAccept.class) ;
     ...
     */
    public LocatorFileAccept() { this(null) ; }
    ...
    {
        log.trace(filenameIRI);
        if (!filenameIRI.substring(0, 7).equals("accept:")) {
            filenameIRI = "accept:*/*:" + filenameIRI;
        }

        // get accept
        int index = filenameIRI.indexOf(":", 7);
        if (index == -1) {
            log.debug("not supported " + filenameIRI);
            return null;
        }
        filenameIRI = filenameIRI.substring(index + 1);
