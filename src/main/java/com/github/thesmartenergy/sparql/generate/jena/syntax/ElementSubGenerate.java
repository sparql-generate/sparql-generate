...
public class ElementSubGenerate extends Element {
    ...
    public ElementSubGenerate(SPARQLGenerateQuery query) {
        this.query = query;
    }

    public SPARQLGenerateQuery getQuery() {
        return query;
    }

    @Override
    public boolean equalTo(Element other, NodeIsomorphismMap isoMap) {
        if (!(other instanceof ElementSubGenerate)) {
            return false;
        }
        ElementSubGenerate el = (ElementSubGenerate) other;
