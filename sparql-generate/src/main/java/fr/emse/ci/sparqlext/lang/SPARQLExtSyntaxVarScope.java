...
public class SPARQLExtSyntaxVarScope {
    ...
    (expr AS v) for BIND and SOURCE                 v is in-scope
    (expr AS v1 v2 v3) for ITERATOR                 v1 v2 v3 are in-scope
    ...
    (expr AS v) for BIND ..                         v is in-scope
    ...
    public static void check(Query query) {
        if (query.getQueryPattern() == null) // DESCRIBE may not have a pattern
        {
            return;
        }
